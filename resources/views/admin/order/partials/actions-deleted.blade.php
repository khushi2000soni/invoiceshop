@can('invoice_print')
<button class="btn btn-icon btn-info print-order-btn p-1 px-2 print-button" data-href="{{route('orders.print', ['order'=> $order->id,'type'=>'deleted'])}}" title="@lang('quickadmin.qa_print')" ><i class="fas fa-print"></i> </button>
@endcan
...
<form action="{{route('orders.restore', $order->id)}}" method="POST" class="restoreForm m-1" id="restoreForm">
    <button class="btn btn-icon btn-info restoreBtn  d-inline-flex align-items-center" type="submit" title="@lang('quickadmin.qa_restore')"><i class="fas fa-trash-restore"></i></button>
</form>
@endcan
